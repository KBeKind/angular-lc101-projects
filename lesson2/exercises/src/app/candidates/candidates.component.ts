...
export class CandidatesComponent implements OnInit {
  ...
  addToCrew(newCrewObj: object) {

    for (let i = 0; i < this.crew.length; i++) {
      if (JSON.stringify(newCrewObj) === JSON.stringify(this.crew[i])) {
        return null;
      }

    }
    this.crew.push(newCrewObj);
  ...
  test(value: object){
    for (let i = 0; i < this.crew.length; i++) {
    if (JSON.stringify(value) === JSON.stringify(this.crew[i])){
      return false;
    }
    }
    return true;
